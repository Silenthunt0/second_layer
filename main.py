...
if not ("TCL_LIBRARY" in os.environ and "TK_LIBRARY" in os.environ):
    try:
        tk.Tk()
    except tk.TclError:
        tk_dir = "tcl" if platform.system() == "Windows" else "lib"
        tk_path = pathlib.Path(base_prefix) / tk_dir
        os.environ["TCL_LIBRARY"] = str(next(tk_path.glob("tcl8.*")))
        os.environ["TK_LIBRARY"] = str(next(tk_path.glob("tk8.*")))
...
def encrypt_text():
    try:
        ...
        other_pub_key = serialization.load_pem_public_key(
            other_pub_key_pem.encode(), backend=default_backend()
        )
...
def decrypt_text():
    try:
        ...
        if "--" not in encrypted_combined:
            raise ValueError(
                "Invalid encrypted format. Expecting '<key_block>--<ciphertext>'"
            )
...
tk.Button(root, text="Copy encrypted", command=copy_encrypted_text).pack(
    side=tk.LEFT, padx=10, pady=10
)
tk.Button(root, text="Paste encrypted", command=paste_encrypted_text).pack(
    side=tk.LEFT, padx=10, pady=10
)
...
tk.Button(frame, text="Encrypt", command=encrypt_text).pack(
    side=tk.LEFT, padx=10, pady=10
)
tk.Button(frame, text="Decrypt", command=decrypt_text).pack(
    side=tk.LEFT, padx=10, pady=10
)
